...
- Omits OmniFocus's generated timestamp/setup preamble from API results so pagination is stable and searches without matching declarations return empty text.
...
### Fixed
- Repeating task completion now distinguishes `completedOccurrenceId` (the completed clone returned by OmniFocus) from `nextOccurrenceId` (the original task, still active). A returned ID alone no longer counts as proof of completion. The tool now correctly advertises `idempotentHint: false`, since retrying an active repeating task completes another occurrence.
...
### Verification
- Release validation on OmniFocus 4.9 (build 187.2.1): 862 tests passed, TypeScript checks and production build passed, dependency audit reported zero vulnerabilities, and live smoke checks passed (50 read checks, 70 mutation/cleanup checks). Both new repeat patterns produced the expected next date and completion IDs; no disposable objects remained.
