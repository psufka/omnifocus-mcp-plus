...
export async function completeTask(taskId: string): Promise<{ success: boolean; id?: string; name?: string; error?: string }> {
  const script = `
    ...
    if (task.taskStatus === Task.Status.Completed) {
      return JSON.stringify({ success: false, error: 'Task is already completed' });
    }
    task.markComplete();
