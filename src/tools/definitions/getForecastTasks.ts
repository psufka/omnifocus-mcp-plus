...
export const schema = z.object({
  ...
  dateMode: z.enum(['direct', 'effective']).optional().describe('Date predicates use dates set directly on tasks, or effective dates inherited from parents/projects. Default: effective.'),
...
export async function handler(args: z.infer<typeof schema>, extra: RequestHandlerExtra<any, any>) {
  try {
    const result = await getForecastTasks({
      includeProjectRoots: args.includeProjectRoots,
